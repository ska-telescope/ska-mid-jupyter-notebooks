...
def test_setup_monitoring(
    telescope_state: TelescopeModel | None, telescope_monitor_plot: TelescopeMononitorPlot
) -> None:
    """
    ...
    """
    assert telescope_state is not None, "Telescope state not loaded"
...
def test_open_inline_dashboard(telescope_state: TelescopeModel | None) -> None:
    """
...
def test_sut_tmc_diagnostics(sut: TangoSUTDeployment) -> None:
    ...
    try:
        ...
        caplog.info(
            f"TMC Central Node dish vcc validation status: {str(tmc.dishvccvalidationstatus)}"
        )
        ...
        caplog.error("Tango connection failed: %s", err_msg)
        assert 0, f"Tango connection failed {err_msg}"
...
def test_dish_lmc_diagnostics(dish_deployments: List[TangoDishDeployment]) -> None:
    ...
    for dish in dish_deployments:
        try:
            ...
            caplog.info(
                f"{dish.dish_id}: DS Manager Operating Mode: {str(ds_manager.operating_mode)}"
            )
            caplog.info(
                f"{dish.dish_id}: DS Manager Indexer Position: {ds_manager.indexerPosition}"
            )
        except tango.ConnectionFailed as terr:
            err_count += 1
            err_msg = terr.args[0].desc.strip()
            caplog.error("Tango connection failed: %s", err_msg)
    assert err_count == 0, f"Tango connection failed {err_count} times"
...
def test_full_diagnostics(
    ...
    for chart in sut.release.sub_charts:
        ...
        except urllib3.exceptions.MaxRetryError as uerr:
            caplog.error("Chart URL error: %s", uerr)
            err_count += 1
    for dish in dish_deployments:
        try:
            caplog.info(f"Dish {dish.dish_id}: Full Diagnostics")
            ...
            caplog.error("Dish URL error: %s", uerr)
            err_count += 1
    assert err_count == 0, f"URL failed {err_count} times"
...
def test_telescope_on(telescope_monitor_plot: TelescopeMononitorPlot) -> None:
    ...
    while on_off != "ON" and retry > 0:
        ...
        on_off = telescope_monitor_plot.on_off_state
        caplog.info("Telescope is %s (%d retries left)", on_off, retry)
...
def test_oda_uri(oda_uri: str) -> None:
    ...
    """
    caplog.info("Check URL %s", oda_uri)
    try:
...
def test_assign_resources(
    ...
    caplog.debug("Assign resources request: %s", json.dumps(json.loads(request_json), indent=2))
